...
def save_multi_agent_dev_version(project_id: str, state: DevelopmentAgentState, zip_path: str, artifact_paths_map: dict, db: Session = None):
    should_close = False
    if db is None:
        db = SessionLocal()
        should_close = True
...
def human_approval(state: DevelopmentAgentState) -> Dict[str, Any]:
    ...
    # Ensure development version is saved in DB if code was generated
    merged_files = state.get("merged_files", {})
    if merged_files:
        db = SessionLocal()
        try:
            ...
            if not latest_ver:
                try:
                    ...
                    print(f"[Human Approval] Auto-saving dev version error: {ex}")
        finally:
            db.close()
...
def post_approval_handler(state: DevelopmentAgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        if not latest_ver and state.get("merged_files"):
            try:
                res = artifact_generator_node(state)
                latest_ver = save_multi_agent_dev_version(
                    project_id=project_id,
                    state=state,
                    zip_path=res["artifact_paths"]["source_zip"],
                    artifact_paths_map=res["artifact_paths"]
                ...
                print(f"[Post Approval] Error auto-saving dev version: {ex}")
