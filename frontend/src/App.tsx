...
const cleanMermaidChart = (rawChart: string): string => {
  ...
  if (cleaned.startsWith('left to right direction') || (cleaned.includes('actor ') && !cleaned.startsWith('graph') && !cleaned.startsWith('flowchart') && !cleaned.startsWith('sequenceDiagram') && !cleaned.startsWith('classDiagram'))) {
...
const MermaidChart: React.FC<{ chart: string; id: string }> = ({ chart, id }) => {
  ...
  useEffect(() => {
    ...
    const uniqueSvgId = `mermaid_svg_${id.replace(/[^a-zA-Z0-9_]/g, '_')}_${Math.random().toString(36).substring(2, 8)}`;
    ...
    try {
      ...
      const renderRes = mermaid.render(uniqueSvgId, cleanedChart);
      if (renderRes && typeof renderRes.then === 'function') {
        renderRes.then(({ svg }) => {
          if (isMounted) {
            setSvgHtml(svg);
            setLoading(false);
        ...
        if (isMounted) {
          setSvgHtml(renderRes as unknown as string);
          setLoading(false);
        }
      }
    } catch (err) {
      console.error("Mermaid sync error for chart", id, err);
      if (isMounted) {
        setLoading(false);
        setRenderError(true);
      }
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 p-6 rounded-lg border border-slate-800 text-xs text-slate-500 flex items-center justify-center gap-2 w-full">
        <RefreshCw className="h-4 w-4 animate-spin text-indigo-500" />
        <span>Rendering UML diagram graphic...</span>
      </div>
    );
  }
  ...
  return (
    <div 
      className="bg-slate-950 p-4 rounded-lg overflow-x-auto border border-slate-800/80 text-xs w-full min-h-[160px] flex items-center justify-center [&>svg]:max-w-full [&>svg]:h-auto [&>svg]:mx-auto [&>svg]:my-0" 
...
export default function App() {
  ...
  const handleSubmitClarificationAnswer = async (questionId: string) => {
    const answer = clarificationAnswers[questionId];
    if (!answer || !answer.trim() || !selectedProjectId || submittingAnswer) return;
    setSubmittingAnswer(true);
    try {
      setToastMessage("Submitting clarification answer and updating requirements...");
      await api.submitClarificationAnswer(selectedProjectId, questionId, answer.trim());
      setClarificationAnswers(prev => ({ ...prev, [questionId]: '' }));
      await fetchProjectDetails(selectedProjectId);
      setToastMessage("Requirements updated with clarification evidence!");
    } catch (err: any) {
      console.error(err);
      alert("Failed to submit clarification answer: " + (err.message || err));
    } finally {
  ...
  const fetchProjectDetails = async (id: string) => {
    ...
    try {
      ...
      if (activeTab !== 'dashboard') {
        ...
        } else if (details.project.current_phase === 'TESTING') {
    ...
    try {
      if (activeTab === 'requirements') {
        ...
        if (reviewStatus === 'APPROVED' && details.project.current_phase === 'DESIGN') {
          ...
          setActiveTab('design');
          setToastMessage("Requirements Approved! Completed all pipeline stages. Transitioned to Design Agent.");
          if (!details.sdd) {
            handleGenerateDesign();
          }
        } else if (reviewStatus === 'APPROVED') {
          setToastMessage(`Stage '${reviewStage || 'Gated Review'}' approved! Moving to next pipeline stage...`);
        }
            ...
            {selectedProjectId && (
              <div className="flex items-center gap-1.5 bg-slate-800/80 border border-slate-700/80 p-1 rounded-lg">
                <span className="text-[10px] font-extrabold text-indigo-400 uppercase tracking-widest px-2 flex items-center gap-1">
                  ...
                  {activeTab === 'design' ? 'Export SDD:' : activeTab === 'development' ? 'Export Dev Artifacts:' : 'Export SRS:'}
                </span>
                {activeTab === 'development' ? (
                  <>
                    <button 
                      onClick={() => window.open(`${API_BASE}/projects/${selectedProjectId}/development/download?api_key=${API_KEY}`, '_blank')}
                      title="Download complete project source ZIP archive"
                    ...
                    <button 
                      onClick={() => window.open(`${API_BASE}/projects/${selectedProjectId}/development/artifacts/download?type=readme&api_key=${API_KEY}`, '_blank')}
                      title="Download Refinement & Architecture README"
                      className="flex items-center gap-1 py-1 px-2.5 bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 hover:text-white rounded text-xs font-bold transition-all border border-indigo-500/30"
                    >
                    ...
                    <button 
                      onClick={() => window.open(`${API_BASE}/projects/${selectedProjectId}/development/artifacts/download?type=manifest&api_key=${API_KEY}`, '_blank')}
                      title="Download Development Manifest JSON"
                      className="flex items-center gap-1 py-1 px-2.5 bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 hover:text-white rounded text-xs font-bold transition-all border border-indigo-500/30"
                    >
              ...
              <div className="flex-1 flex overflow-hidden">
                ...
                <div className="flex-1 flex flex-col bg-[#0f1422] border-r border-slate-800 min-w-0">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-4">
                    ...
                    {/* Dynamic Pending Clarification Questions UI */}
                    {projectDetails?.agent_state?.pending_questions?.length > 0 && (
                      <div className="space-y-3 bg-[#111726] border border-amber-500/30 rounded-xl p-4 shadow-lg text-left">
                        <div className="flex items-center justify-between border-b border-amber-500/20 pb-2">
                          <span className="text-xs font-bold text-amber-400 uppercase tracking-widest flex items-center gap-1.5">
                            <HelpCircle className="h-4 w-4 text-amber-400" />
                            Contextual Clarification Required ({projectDetails.agent_state.pending_questions.length})
                          </span>
                        ...
                        <p className="text-xs text-slate-400">
                          The Requirement Agent identified unresolved project ambiguities. Please answer to update requirements and resume analysis.
                        </p>
                        <div className="space-y-3">
                          {projectDetails.agent_state.pending_questions.map((q: any, idx: number) => (
                            <div key={q.id || idx} className="bg-[#0b0f19] border border-slate-800 rounded-lg p-3 space-y-2">
                              <div className="flex items-center justify-between">
                                <span className="text-[10px] font-extrabold text-indigo-400">{q.id}</span>
                                <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded ${
                                  q.priority === 'high' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                                }`}>
                                  {q.priority} priority
                                </span>
                              </div>
                              ...
                              {q.reason && (
                                <p className="text-[11px] text-slate-400 italic">Reason: {q.reason}</p>
                              )}
                              <div className="flex items-center gap-2 pt-1">
                                <input
                                  ...
                                  placeholder="Type human clarification answer..."
                                  className="flex-1 bg-[#161c2e] border border-slate-700/70 rounded-lg px-3 py-1.5 text-xs text-slate-100 focus:outline-none focus:ring-1 focus:ring-amber-500"
                                />
                                <button
                                  type="button"
                                  onClick={() => handleSubmitClarificationAnswer(q.id)}
                                  disabled={submittingAnswer || !clarificationAnswers[q.id]?.trim()}
                                  className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-slate-950 font-bold text-xs rounded-lg transition-all shadow-md shrink-0"
                                >
                                  Submit Answer
                                </button>
                              </div>
                ...
                <div className="w-80 flex flex-col bg-[#111827] border-l border-slate-800 shrink-0 justify-between">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    {/* Approve & Move to Design Agent Button (Below Gated Approval Pipeline) */}
                    {projectDetails.project.status !== 'APPROVED' && projectDetails.project.current_phase === 'REQUIREMENT' && (
                      <div className="pt-2">
                        <button 
                          onClick={async () => {
                            if (!selectedProjectId || sendingChat) return;
                            setSendingChat(true);
                            setToastMessage("Approving Requirements & Transitioning to Design Agent...");
                            try {
                              await api.submitReview(selectedProjectId, "APPROVED", "Requirements Approved — Proceeding to System Architecture Design", "Lead Architect", "FINALIZATION");
                              const details = await api.getProjectStatus(selectedProjectId);
                              setProjectDetails(details);
                              setActiveTab('design');
                              setToastMessage("Requirements Approved! Transitioned to Design Agent.");
                              if (!details.sdd) {
                                handleGenerateDesign();
                              }
                            } catch (err) {
                          ...
                          disabled={sendingChat}
                          className="w-full py-3 px-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 disabled:opacity-50 text-white rounded-xl font-extrabold text-xs shadow-xl shadow-emerald-600/30 flex items-center justify-center gap-2 uppercase tracking-wider transition-all cursor-pointer border border-emerald-400/30 hover:scale-[1.01]"
                        >
                          <CheckCircle2 className="h-4.5 w-4.5 text-emerald-200" />
                          Approve & Move to Design Agent &rarr;
                        </button>
                        <span className="text-[10px] text-slate-400 block text-center mt-2 font-medium">
                          Click to sign off on requirements and unlock Design Agent
                        </span>
                      </div>
                    )}
                            ...
                            return (
                              <div className="bg-[#111827] border border-slate-800 rounded-xl p-5 shadow-sm space-y-6 animate-fade-in">
                                ...
                                <div className="space-y-8 pt-2">
                                  ...
                                  .filter(item => activeDiagramTab === 'all' || activeDiagramTab === item.id || !activeDiagramTab)
                                  .map((item) => {
                                    const chart = sdd[item.field];
                                    return (
                                      <div key={item.id} id={`ws_diag_${item.id}`} className="space-y-3 bg-slate-900/40 border border-slate-800/80 p-5 rounded-xl shadow-md">
                                        <h5 className="font-extrabold text-xs text-indigo-300 uppercase tracking-wider flex items-center gap-2 border-b border-slate-800/60 pb-2">
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="space-y-8 pt-2">
                            ...
                            .map((item) => {
                              const chart = sdd[item.field];
                              return (
                                <div key={item.id} id={`diag_sec_${item.id}`} className="space-y-3 bg-slate-900/30 border border-slate-800/80 p-5 rounded-xl shadow-md">
                                  <h5 className="font-extrabold text-xs text-indigo-300 uppercase tracking-wider flex items-center gap-2 border-b border-slate-800/60 pb-2">
                                    <Sparkles className="h-3.5 w-3.5 text-indigo-400" />
                                    {item.title}
                                  </h5>
