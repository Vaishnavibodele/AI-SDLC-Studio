...
export const api = {
  ...
  async submitClarificationAnswer(projectId: string, questionId: string, answer: string) {
    const res = await apiFetch(`${API_BASE}/projects/${projectId}/requirements/answer`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ question_id: questionId, answer })
    });
    if (!res.ok) throw new Error('Failed to submit clarification answer');
    return res.json();
  },
