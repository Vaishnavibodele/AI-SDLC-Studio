...
@app.post("/api/projects/{project_id}/requirements/answer")
def submit_clarification_answer(project_id: str, payload: schemas.ClarificationAnswerSubmit, db: Session = Depends(get_db)):
    proj = project_service.get_project(db, project_id)
    if not proj:
        raise HTTPException(status_code=404, detail="Project not found")
    try:
        return agent_service.submit_clarification_answer(db, project_id, payload.question_id, payload.answer)
    except Exception as e:
        import traceback
        traceback.print_exc()
...
def submit_development_approval(project_id: str, review: schemas.DevelopmentReviewSubmit, db: Session = Depends(get_db)):
    ...
    latest_dev = development_service.ensure_latest_development_version(db, project_id)
    if not latest_dev:
