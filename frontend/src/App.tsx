  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      if (activeTab === 'requirements') {
        ...
        if (reviewStatus === 'APPROVED' && details.project.current_phase === 'DEVELOPMENT') {
          setActiveTab('development');
          setToastMessage("Software Design Document Approved successfully. Development Phase has started.");
        }
      }
          ...
          <nav className="p-4 space-y-1">
            ...
            <div className="space-y-1 py-1">
              ...
              <button disabled className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-semibold text-slate-600 cursor-not-allowed">
                ...
                Testing Agent <span className="ml-auto text-[9px] bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800">LOCKED</span>
              </button>
              <button disabled className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-semibold text-slate-600 cursor-not-allowed">
                ...
                {(generatingDesign || projectDetails.design_agent_state.phase === 'generating') ? (
                  ...
                  <div className="flex-1 flex overflow-hidden w-full pb-16 min-h-[500px]">
                    ...
                    <div className="w-64 border-r border-slate-800 overflow-y-auto pr-4 space-y-1 bg-[#111827]/10 p-3 rounded-xl mr-6 shrink-0">
                      ...
                      {projectDetails.design_agent_state.phase === 'awaiting_approval' && (
                    ...
                    <div className="flex-1 overflow-y-auto space-y-6 pr-2">
                      ...
                      {(() => {
                        ...
                        switch (activeSddGroup) {
                          ...
                          case 'uml':
                            return (
                              <div className="bg-[#111827] border border-slate-800 rounded-xl p-5 shadow-sm space-y-4 animate-fade-in">
                                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                                  <div className="flex items-center gap-2 text-indigo-400 font-bold text-xs uppercase tracking-widest">
                                    <Sparkles className="h-4 w-4" />
                                    <span>10-16. UML Diagram Visualizations</span>
                                  </div>
                                </div>
                                
                                <div className="flex flex-wrap gap-1.5 border-b border-slate-800 pb-2">
                                  {[
                                    { id: 'context', label: 'System Context', field: 'system_context_diagram_mermaid' },
                                    { id: 'usecase', label: 'Use Cases', field: 'use_case_diagram_mermaid' },
                                    { id: 'component', label: 'Components', field: 'component_diagram_mermaid' },
                                    { id: 'class', label: 'Class structure', field: 'class_diagram_mermaid' },
                                    { id: 'sequence', label: 'Sequence flow', field: 'sequence_diagram_mermaid' },
                                    { id: 'activity', label: 'Activities', field: 'activity_diagram_mermaid' },
                                    { id: 'er', label: 'Entity Relations (ER)', field: 'er_diagram_mermaid' },
                                    { id: 'deployment', label: 'Deployment Nodes', field: 'deployment_diagram_mermaid' },
                                    { id: 'flow', label: 'Process Flow', field: 'flow_diagram_mermaid' },
                                    { id: 'dbrel', label: 'DB relationships', field: 'db_relationship_diagram_mermaid' },
                                  ].map((tab) => (
                                    <button
                                      key={tab.id}
                                ...
                                <div className="pt-2">
                                  {(() => {
                                    const diagramConfig: Record<string, string> = {
                                      context: sdd.system_context_diagram_mermaid,
                                      usecase: sdd.use_case_diagram_mermaid,
                                      component: sdd.component_diagram_mermaid,
                                      class: sdd.class_diagram_mermaid,
                                      sequence: sdd.sequence_diagram_mermaid,
                                      activity: sdd.activity_diagram_mermaid,
                                      er: sdd.er_diagram_mermaid,
                                      deployment: sdd.deployment_diagram_mermaid,
                                      flow: sdd.flow_diagram_mermaid,
                                      dbrel: sdd.db_relationship_diagram_mermaid,
                                    };
                                    const chart = diagramConfig[activeDiagramTab] || '';
                                    return chart ? <MermaidChart chart={chart} id={activeDiagramTab} /> : <div className="text-slate-500 text-xs italic">No diagram data.</div>;
                                  })()}
                                </div>
                              </div>
                            );
