...
class ClarificationQuestion(BaseModel):
    id: str = Field(..., description="Unique question identifier, e.g. GAP-001")
    question: str = Field(..., description="Dynamic contextual clarification question")
    reason: str = Field(..., description="Explanation of why this information is required for the project")
    related_requirements: List[str] = Field(default_factory=list, description="IDs or statements of related requirements")
...
class QualityResult(BaseModel):
    valid: bool
    scores: Dict[str, int]  # completeness, consistency, clarity, testability, traceability, coverage, grounding
    overall_score: int
    issues: List[str] = []
    warnings: List[str] = []
