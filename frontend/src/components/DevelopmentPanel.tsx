      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#0b0f19]">
        ...
        <div className="h-14 border-b border-slate-800 flex items-center justify-between px-8 bg-[#111827] shrink-0">
          <div className="flex items-center gap-3">
            ...
            {selectedFile ? (
              <span className="font-mono text-xs font-extrabold text-slate-200">{selectedFile}</span>
            ) : selectedReport ? (
              <span className="font-mono text-xs font-extrabold text-slate-200">{getReportName(selectedReport)}</span>
            ) : (
              <span className="text-xs italic text-slate-500">No file selected</span>
            )}
          </div>
        ...
        <div className="flex-1 p-8 overflow-y-auto space-y-6">
          ...
          {manifest.length > 0 && (
            <div className="space-y-2">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-500">Stage 4-8: Multi-Agent Parallel Scaffolding</span>
              <div className="grid grid-cols-6 gap-3">
                {[
                  { name: "DatabaseDeveloperAgent", label: "Database Agent", icon: <Database className="h-4 w-4" /> },
                  { name: "BackendDeveloperAgent", label: "Backend Agent", icon: <Code className="h-4 w-4" /> },
                  { name: "FrontendDeveloperAgent", label: "Frontend Agent", icon: <Layers className="h-4 w-4" /> },
                  { name: "APIIntegrationAgent", label: "API Integration", icon: <Activity className="h-4 w-4" /> },
                  { name: "DocumentationAgent", label: "Documentation", icon: <FileJson className="h-4 w-4" /> },
                  { name: "SelfReviewAgent", label: "Self Review", icon: <ShieldCheck className="h-4 w-4" /> }
                ].map((ag) => {
                  ...
                  return (
                    <div key={ag.name} className="bg-slate-900 border border-slate-800 p-3 rounded-xl flex flex-col justify-between gap-3 min-h-[110px] shadow-sm">
                      ...
                      <div className="text-[10px] text-slate-500 space-y-0.5 font-semibold">
                        ...
                        {details.retry_count !== undefined && (
                          <div>Retries: <span className="text-slate-300">{details.retry_count}</span></div>
                        )}
                        {details.files_generated && details.files_generated.length > 0 && (
                          <div className="truncate">Files: <span className="text-slate-300">{details.files_generated.length}</span></div>
                        )}
          ...
          {Object.keys(selfReviewReport).length > 0 && (
            <div className="bg-[#121625]/60 border border-slate-800 p-5 rounded-2xl space-y-4">
              ...
              {selfReviewReport.missing_components?.length > 0 && (
                <div className="bg-amber-500/10 border border-amber-500/25 p-3 rounded-lg text-xs text-amber-400 flex flex-col gap-1">
                  <span className="font-extrabold text-[10px] uppercase">Missing Components Detected:</span>
                  <ul className="list-disc list-inside">
                    {selfReviewReport.missing_components.map((c: string, i: number) => (
                      <li key={i}>{c}</li>
                    ))}
                  </ul>
                </div>
        ...
        <div className="p-5 border-b border-slate-800 bg-[#0f1422]">
          ...
          <div className="space-y-4">
            {[
              ...
              { id: 'WAITING_FOR_REVIEW', label: '9. Human Review Gate', activePhases: ['WAITING_FOR_REVIEW', 'ERROR'] },
              { id: 'READY_FOR_TESTING', label: '10. Ready for Testing', activePhases: ['READY_FOR_TESTING'] }
            ].map((s) => {
          ...
          <div className="p-5 border-b border-slate-800 bg-[#121625]/30 space-y-4">
            <div className="bg-indigo-650/15 border border-indigo-500/20 p-4 rounded-xl space-y-1">
              ...
              <p className="text-[10px] text-slate-400 leading-normal">
                {phase === 'ERROR' 
                  ...
                  : "Review codebase output and scores. Approve to sign off, or select affected modules to reject."}
              </p>
            </div>
            ...
            <div className="space-y-3.5">
              ...
              <div className="space-y-2 bg-slate-950 p-3 rounded-xl border border-slate-800">
                ...
                <span className="text-[9px] text-slate-500 block leading-tight mb-2">Check modules to regenerate on Reject. Leave blank to regenerate all.</span>
                <div className="space-y-1.5">
                  {[
